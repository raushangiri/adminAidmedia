...
const DirectWithdrawRequest = () => {
  ...
  const getallwithdraw = () => {
    fetch("https://backend-87hw.onrender.com/referwithdrawrequest").then(
      (item) => {
        item.json().then((res) => {
          setdata(res.Withdrawrequest);
          setLoading(false);
        });
      }
    );
    ...
    <>
      <tbody>
        <tr>
          ...
          <td className="text-white">
            <button className="btn btn-info" onClick={Approvedirectwithdraw}>
              Mark as Paid
            </button>
          </td>
        </tr>
